...
class PlayState extends MusicBeatState
{
	...
	public function bruhevent(numberlol)
	{
		var bruh:FlxSprite = new FlxSprite();
		if (numberlol == 1)
		{
			bruh.loadGraphic(Paths.image('thud/spanchblap1'));
		}
		else if (numberlol == 2)
		{
			bruh.loadGraphic(Paths.image('thud/spanchblap2'));
		}
		else if (numberlol == 3)
		{
			bruh.loadGraphic(Paths.image('thud/spanchblap3'));
		}
		else if (numberlol == 4)
		{
			bruh.loadGraphic(Paths.image('thud/spanchblap4'));
		}
		else if (numberlol == 5)
		{
			bruh.loadGraphic(Paths.image('thud/spanchblap5'));
		}
		...
		bruh.screenCenter();
		add(bruh);
		FlxTween.tween(bruh, {y: bruh.y += 100, alpha: 0}, Conductor.crochet / 1000, {
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'cocoa' | 'eggnog':
			{
					...
					var justlucky:FlxSprite = new FlxSprite();
					justlucky.loadGraphic(Paths.image('christmas/justlucky'));
					justlucky.antialiasing = true;
					justlucky.active = false;
					justlucky.scrollFactor.set();
					justlucky.screenCenter();
					add(justlucky);
			}
			...
			case 'expbrbrbbrkjjji':
			{
				curStage = 'mystage';
				
				var bg:FlxSprite = new FlxSprite(-5, -9);
				bg.loadGraphic(Paths.image('penta/JonBg'));
				add(bg);
				bg.updateHitbox();
				bg.active = false;
				bg.antialiasing = true;
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			FlxG.sound.play(Paths.sound('thud'));
			bruhevent(FlxG.random.int(1,5));
